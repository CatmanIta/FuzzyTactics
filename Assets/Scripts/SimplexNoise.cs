...
static public class SimplexNoise {
	...
	static public float noise(float xin, float yin, float zin){
		...
		// Skew the input space to determine which simplex cell we're in
	    F3 = 1.0f/3.0f; // Skew factor
	    s = (xin+yin+zin)*F3;
	    i = fastfloor(xin+s);
    	j = fastfloor(yin+s);
     	k = fastfloor(zin+s);
	
	    G3 = 1.0f/6.0f; // Unskew factor
	    t = (i+j+k)*G3;
	    ...
	    if(t0<0){
	        ...
	        t0 *= t0;
	        n0 = t0 * t0 * dot(grad4[gi0], x0, y0, z0);
	    }
	    ...
	    if(t1<0){
	        ...
	        t1 *= t1;
	        n1 = t1 * t1 * dot(grad4[gi1], x1, y1, z1);
	    }
	    ...
	    if(t2<0){
	        ...
	        t2 *= t2;
	        n2 = t2 * t2 * dot(grad4[gi2], x2, y2, z2);
	    }
	    ...
	    if(t3<0){
	        ...
	        t3 *= t3;
	        n3 = t3 * t3 * dot(grad4[gi3], x3, y3, z3);
	    }
	...
	// Called to create 3D noise
	// returns the chosen noise value dependent on the 3D position
	// The return is scaled in [0,1]
	static public float noiseOctaves(int octaves, float x, float y, float z){
	    float value = 0.0f;
	    int i;
	    for(i=0; i<octaves; i++){
	        value += noise(
	...
	// Called to create 2D noise
	// returns the chosen noise value dependent on the 2D position
	// The return is scaled in [-1,1]  
	//static public float noiseOctaves(int octaves, float x, float z){ return noiseOctaves(octaves,x,z,0.0f,1.0f);}
	//static public float noiseOctaves(int octaves, float x, float z, float phase){ return noiseOctaves(octaves,x,z,phase,1.0f);}
	static public float noiseOctaves(int octaves, float x, float z, float phase, float frequency){
	    float value = 0.0f;  
	    int i; 
	    for(i=0; i<octaves; i++){
	        value += noise(
	            frequency*(phase+x)*Mathf.Pow(2, i),
	            frequency*(phase+z)*Mathf.Pow(2, i)
	        );
	    }
	    return value;
	}
